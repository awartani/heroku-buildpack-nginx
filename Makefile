...
build-heroku-20:
	...
	@docker run -v $(shell pwd):/buildpack -v $(shell pwd)/../librdkafka:/buildpack/librdkafka -v $(shell pwd)/../ngx_kafka_module:/buildpack/ngx_kafka_module --rm -it -e "STACK=heroku-20" -w /buildpack heroku/heroku:20-build scripts/build_nginx /buildpack/nginx-heroku-20.tgz
